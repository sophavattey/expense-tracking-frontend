...
export default function ExpenseFormPage() {
  ...
  useEffect(() => {
    ...
    expenseService.getById(Number(editId))
      .then(exp => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && editId) await expenseService.update(Number(editId), payload);
  ...
  const loading    = catsLoading || loadingExpense;
  ...
  // ── Preview panel (shared between mobile sheet and desktop sidebar) ──
  ...
  return (
    <>
      ...
      <div className="w-full space-y-4 sm:space-y-6" style={{ animation: "slideUp 0.3s ease both" }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-blue-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest">
                {isEdit ? "Editing expense" : "New expense"}
              </p>
        ...
        {/* ── Live mini-preview bar (mobile) — shows when amount is filled ── */}
        {numAmt > 0 && (
          <div className="xl:hidden flex items-center gap-3 bg-blue-600 rounded-2xl px-4 py-3"
            ...
            <div className="w-8 h-8 rounded-xl flex items-center justify-center text-lg shrink-0"
              style={selectedCategory
                ? { backgroundColor: selectedCategory.color + "50" }
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="xl:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-blue-100 p-4 sm:p-6 shadow-sm space-y-4">
              ...
              {/* Date + merchant — stack on mobile, side by side on sm+ */}
            ...
            <div className="bg-white rounded-2xl border border-blue-100 p-4 sm:p-6 shadow-sm">
              <Field label="Category *" error={errors.category}>
                {/* 2 cols on mobile, 3 on sm, 4 on xl */}
          ...
          {/* ══ Desktop sticky preview sidebar ══ */}
