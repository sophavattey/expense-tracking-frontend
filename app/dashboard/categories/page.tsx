  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-lg"
        ...
        <div className="w-10 h-1 bg-blue-100 rounded-full mx-auto mt-4 mb-2 sm:hidden" />
        <div className="p-5 sm:p-7">
          <div className="flex items-center justify-between mb-5">
            <div>
              <p className="text-blue-400 text-[10px] font-bold uppercase tracking-widest">{initial ? "Editing" : "New"}</p>
              <h2 className="text-blue-800 font-black text-xl sm:text-2xl font-['Sora',sans-serif]">
                {initial ? "Edit Category" : "Create Category"}
              </h2>
            </div>
            <button onClick={onClose} className="w-9 h-9 rounded-xl bg-blue-50 hover:bg-blue-100 flex items-center justify-center text-blue-400 transition-colors">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>

          {/* Live preview */}
          <div className="flex items-center gap-4 rounded-2xl p-4 mb-5 border border-blue-100" style={{ backgroundColor: color + "08" }}>
            ...
            <div>
              <p className="text-blue-800 font-black text-lg leading-tight">{name || "Category name"}</p>
              <p className="text-blue-300 text-xs mt-0.5">Live preview</p>
              <div className="flex items-center gap-1.5 mt-1">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: color }} />
                <span className="text-xs font-mono text-blue-400">{color}</span>
              </div>
            </div>
          </div>

          <div className="mb-4">
            <label className="block text-blue-600 text-[10px] font-bold uppercase tracking-widest mb-2">Name *</label>
          ...
          {/* Emoji picker — intentionally kept as emojis, these are user-facing category icons */}
          <div className="mb-4">
            <label className="block text-blue-600 text-[10px] font-bold uppercase tracking-widest mb-2">Icon</label>
            <div className="grid grid-cols-8 sm:grid-cols-10 gap-1.5">
              {EMOJI_OPTIONS.map(e => (
                <button key={e} type="button" onClick={() => setIcon(e)}
                  className={`h-10 rounded-xl text-xl flex items-center justify-center transition-all ${icon === e ? "scale-110 shadow-md" : "bg-blue-50 hover:bg-blue-100"}`}
          ...
          <div className="mb-5">
            <label className="block text-blue-600 text-[10px] font-bold uppercase tracking-widest mb-2">Color</label>
            <div className="flex flex-wrap gap-2.5">
              {COLOR_OPTIONS.map(c => (
                <button key={c} type="button" onClick={() => setColor(c)}
                  className={`w-9 h-9 rounded-xl transition-all ${color === c ? "scale-125 shadow-lg ring-2 ring-offset-2 ring-blue-300" : "hover:scale-110"}`}
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => onSave({ name: name.trim(), icon, color })} disabled={saving || !name.trim()}
              className="flex-1 py-3.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-lg hover:shadow-blue-600/25">
              {saving
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-sm p-6 sm:p-7"
        ...
        <div className="text-center mb-5">
          {/* Category icon in delete modal is user-defined, kept as emoji */}
          <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center text-3xl"
            style={{ backgroundColor: category.color + "20" }}>{category.icon}</div>
          <h3 className="text-blue-800 font-black text-xl font-['Sora',sans-serif]">Delete "{category.name}"?</h3>
          {!isBlocked && (
        ...
        <div className="flex gap-3">
          <button onClick={onClose} disabled={deleting}
            className="flex-1 py-3.5 rounded-xl border-2 border-blue-100 text-blue-500 font-bold text-sm hover:bg-blue-50 transition-all disabled:opacity-50">
            {isBlocked ? "Close" : "Cancel"}
          </button>
          {!isBlocked && (
            <button onClick={onConfirm} disabled={deleting}
              className="flex-1 py-3.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {deleting
...
function CategoryCard({ category, onEdit, onDelete }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-blue-100 p-3.5 sm:p-4 flex items-center gap-3 sm:gap-4 hover:shadow-md hover:border-blue-200 transition-all group">
      {/* Category icon is user-defined emoji — kept intentionally */}
      ...
      <div className="flex-1 min-w-0">
        <p className="text-blue-800 font-bold text-sm sm:text-base truncate leading-tight">{category.name}</p>
        <div className="flex items-center gap-1.5 mt-0.5">
          <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: category.color }} />
          <span className="text-blue-400 text-xs">{category.isDefault ? "System · Read-only" : "Custom"}</span>
        </div>
      </div>
      {isEditable ? (
        <div className="flex items-center gap-1.5 shrink-0 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(category)} aria-label="Edit"
            className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-500 transition-colors flex items-center gap-1">
        ...
        <span className="text-[10px] font-bold bg-blue-50 text-blue-300 px-2 py-1 rounded-lg shrink-0">DEFAULT</span>
      )}
    </div>
  );
}
      ...
      <div className="w-full space-y-4 sm:space-y-6">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h1 className="text-blue-800 font-black text-2xl sm:text-3xl font-['Sora',sans-serif] leading-tight mt-0.5">Categories</h1>
            <p className="text-blue-400 text-xs sm:text-sm mt-1 hidden sm:block">Organize your spending into meaningful groups</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            { Icon: Tag,        value: categories.length, label: "Total",    color: "bg-blue-50",   iconColor: "text-blue-500",  desktopOnly: false },
            { Icon: Sparkles,   value: customs.length,    label: "Custom",   color: "bg-indigo-50", iconColor: "text-indigo-500", desktopOnly: false },
            { Icon: LayoutGrid, value: defaults.length,   label: "Defaults", color: "bg-blue-50",   iconColor: "text-blue-400",  desktopOnly: true  },
          ].map(({ Icon, value, label, color, iconColor, desktopOnly }) => (
            <div key={label} className={`bg-white rounded-2xl border border-blue-100 p-4 sm:p-5 flex items-center gap-3 sm:gap-4 ${desktopOnly ? "hidden sm:flex" : ""}`}>
              ...
              <div>
                <p className="text-blue-800 font-black text-xl sm:text-2xl font-['Sora',sans-serif] leading-none">{value}</p>
                <p className="text-blue-400 text-xs mt-0.5">{label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="bg-white rounded-2xl border border-blue-50 p-3.5 flex items-center gap-3 animate-pulse">
                <div className="w-11 h-11 rounded-2xl bg-blue-100 shrink-0" />
                <div className="flex-1"><div className="h-4 bg-blue-100 rounded-lg w-32 mb-2" /><div className="h-3 bg-blue-50 rounded-lg w-20" /></div>
              </div>
            ))}
          </div>
        )}
        ...
        {!loading && (
          <section>
            <div className="mb-3">
              <p className="text-blue-800 font-bold text-sm sm:text-base">Your Categories</p>
              <p className="text-blue-400 text-xs mt-0.5">{customs.length} custom {customs.length === 1 ? "category" : "categories"}</p>
            </div>
            {customs.length === 0 ? (
              <button onClick={() => { setModalError(null); setShowCreate(true); }}
                className="w-full bg-blue-50 hover:bg-blue-100 border-2 border-dashed border-blue-200 hover:border-blue-300 rounded-2xl p-8 sm:p-10 text-center transition-all group">
                <div className="w-14 h-14 rounded-2xl bg-white border border-blue-100 flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform shadow-sm">
                  <Tag size={24} className="text-blue-400" strokeWidth={1.75} />
                </div>
                <p className="text-blue-600 font-bold text-sm sm:text-base">Create your first category</p>
                <p className="text-blue-400 text-xs sm:text-sm mt-1">Organize your spending your way</p>
              </button>
            ) : (
        ...
        {!loading && defaults.length > 0 && (
          <section>
            <div className="mb-3">
              <p className="text-blue-800 font-bold text-sm sm:text-base">System Defaults</p>
              <p className="text-blue-400 text-xs mt-0.5">Read-only · {defaults.length} categories</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
