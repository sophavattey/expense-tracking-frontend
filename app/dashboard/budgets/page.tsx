...
import { Users, Eye, Wallet, Trash2 } from "lucide-react";
...
function PeriodBadge({ period }: { period: BudgetPeriod }) {
  ...
  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border ${styles[period]}`}>{period}</span>;
}
...
/* ── Budget card ── */
function BudgetCard({ status, pref, onEdit, onDelete, canEdit }: {
  status: BudgetStatus; pref: "USD" | "KHR";
  onEdit: (s: BudgetStatus) => void; onDelete: (s: BudgetStatus) => void;
  canEdit: boolean;
}) {
  ...
  return (
    <div className={`bg-white rounded-2xl border shadow-sm p-5 transition-all hover:shadow-md group ${color.borderClass} ${color.bgTintClass}`}>
      ...
      <BudgetProgressBar pct={status.percentage} color={color} />
...
function BudgetModal({ editStatus, categories, onSave, onClose, saving, error: saveError }: {
  editStatus: BudgetStatus | null; categories: Category[];
  onSave: (data: BudgetRequest) => Promise<void>; onClose: () => void;
  ...
  const [limitValue,    setLimitValue]    = useState(() => editStatus ? String(editStatus.limitUsd) : "");
  ...
  const handleSave = async () => {
    if (!validate()) return;
    await onSave({
      categoryId, period, inputCurrency,
...
export default function BudgetsPage() {
  ...
  const isOwner  = !isGroup || activeContext.role === "OWNER";
  const canEdit  = isOwner;

  const { summary, loading, error, createBudget, updateBudget, deleteBudget } = useBudgets({ groupId });
  ...
  const overallColor = getBudgetColor(
    summary && summary.totalLimitUsd > 0
      ? Math.round((summary.totalSpentUsd / summary.totalLimitUsd) * 100) : 0
  );
  ...
  return (
    <>
      ...
      <div className="w-full space-y-5" style={{ animation: "slideUp 0.3s ease both" }}>
        ...
        {isGroup && !isOwner && (
          <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-100 text-indigo-600 text-sm px-4 py-3 rounded-2xl">
            <div className="w-8 h-8 rounded-xl bg-indigo-100 flex items-center justify-center shrink-0">
              <Eye size={15} className="text-indigo-500" strokeWidth={2} />
            </div>
            <p>Only the group owner can create or modify budgets. You can view the group's budget progress here.</p>
          </div>
        )}
        ...
        {!loading && summary && summary.totalBudgets > 0 && (() => {
          const rawPct = summary.totalLimitUsd > 0 ? Math.round((summary.totalSpentUsd / summary.totalLimitUsd) * 100) : 0;
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map(s => (
              <BudgetCard key={s.id} status={s} pref={pref} onEdit={openEdit} onDelete={setDeleteTarget} canEdit={canEdit} />
            ))}
          </div>
        )}
