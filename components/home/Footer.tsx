...
import { MapPin, Lock, TrendingDown, Target, Users, ArrowLeftRight } from "lucide-react";
  ...
  return (
    <footer id="contact" className="bg-gray-900 pt-16 pb-8">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-4 gap-10 mb-12">
          ...
          <div className="md:col-span-1">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              <span className="inline-flex items-center gap-1.5 bg-gray-800 border border-gray-700 text-gray-400 text-[11px] font-semibold px-2.5 py-1 rounded-lg">
                <MapPin size={11} strokeWidth={2} />Made in Cambodia
              </span>
              <span className="inline-flex items-center gap-1.5 bg-gray-800 border border-gray-700 text-gray-400 text-[11px] font-semibold px-2.5 py-1 rounded-lg">
                <Lock size={11} strokeWidth={2} />Secure
              </span>
            </div>
            <div className="flex gap-2.5">
              {[
                ...
                <a key={s.label} href="#" aria-label={s.label}
                  className="w-8 h-8 rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-gray-600 flex items-center justify-center transition-all">
          ...
          {footerLinks.map((col) => (
            <div key={col.title}>
              <p className="text-gray-400 text-[11px] font-bold uppercase tracking-widest mb-4">{col.title}</p>
              <ul className="space-y-2.5">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="text-gray-500 text-sm hover:text-white transition-colors duration-200">
                      {link.label}
                    </a>
                  </li>
                ))}
        ...
        {/* Feature strip */}
        <div className="border-t border-gray-800 border-b py-6 mb-8">
